...
public class LocationDAO {
	 ...
	 * +"FROM seoul_location)) " +"WHERE num BETWEEN #{start} AND #{end}") public
	 * List<LocationVO> locationListData(Map map);
	 * 
	 * @Select("SELECT CEIL(COUNT(*)/20.0) FROM seoul_location") public int
	 * locationTotalPage();
	 * 
	 * @Select("SELECT * FROM seoul_location " +"WHERE no=#{no}") public LocationVO
	 * locationDetailData(int no);
	 */
